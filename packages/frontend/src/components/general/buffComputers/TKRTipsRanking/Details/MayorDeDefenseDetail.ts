...
export const MayorDeDefenseDetail = z.function()
  ...
  .implement((eg: ExtendedGeneralType, bp: BuffParamsType, am: AttributeMultipliersType) => {
    ...
    const aes = AscendingBuffs(eg, generalUseCase.enum.Mayor, bp, am, DeDefenseBuff);

    if(DEBUG) {
      console.log(`MayorDeDefenseDetail: ${eg.name}: bas: ${bas}, bss: ${bss}, speciality: ${speciality} aes: ${aes}`);
    }
