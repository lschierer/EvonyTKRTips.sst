...
export const CovenantBuffs = (
  ...
  if (DEBUG) {
    console.log(`CovenantBuffs for ${eg.name} ${useCase} start `);
  }
  let accumulator = 0;
        ...
        case CovenantAttributeCategory.enum['Civilization Covenant']:
          if (
            ...
            if (!covenantInstance.type.localeCompare('personal')) {
              ...
              if (DEBUG) {
                console.log(
                  `${eg.name} ${useCase} ${covenantInstance.category} additional ${additional}`,
                );
              }
              ...
              if (DEBUG) {
                console.log(
                  `${eg.name} ${useCase} ${covenantInstance.category} is passive, skipping`,
                );
              }
            }
          }
