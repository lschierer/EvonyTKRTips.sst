...
import { MayorPvPAttackAttributeMultipliers } from '../../TKRAttributeRanking';
  ...
  .implement(
    (
      ...
      if (!UseCase.localeCompare(generalUseCase.enum.all)) {
        ...
        if (!generalSpecialists.enum.Archers.localeCompare(eg.score_as)) {
          return ReinforcementAttackPvPBase(
            eg,
            display,
            bp,
