        ...
        if (tb.attribute === undefined || tb.attribute === null) {
          ...
          if (!Attribute.enum.March_Size_Capacity.localeCompare(tb.attribute)) {
            if (!UnitSchema.enum.percentage.localeCompare(tb.value.unit)) {
              const multiplier = SiegePvPAttackAttributeMultipliers?.Offensive.MarchSizeIncrease ?? 0;
            ...
            if (!UnitSchema.enum.percentage.localeCompare(tb.value.unit)) {
              const multiplier = SiegePvPAttackAttributeMultipliers?.Preservation.Death2Souls ?? 0;
              const additional = tb.value.number * multiplier;
              if (DEBUG_GBUFF) {
                console.log(
                  `GroundPvPBuff: ${buffName} from ${generalName} adds ${additional} to ${score}`
            ...
            if (!UnitSchema.enum.percentage.localeCompare(tb.value.unit)) {
              const multiplier = SiegePvPAttackAttributeMultipliers?.Preservation.Death2Wounded ?? 0;
              const additional = tb.value.number * multiplier;
              if (DEBUG_GBUFF) {
                console.log(
                  `GroundPvPBuff: ${buffName} from ${generalName} adds ${additional} to ${score}`
            ...
            if (tb.condition !== undefined && tb.condition !== null && (
              ...
              if (!UnitSchema.enum.percentage.localeCompare(tb.value.unit)) {
                ...
                const multiplier = SiegePvPAttackAttributeMultipliers?.AttackingAttackDebuff.ReduceAllAttack ?? 0;
                const additional = tb.value.number * multiplier;
                if (DEBUG_GBUFF) {
                  console.log(
                    `GroundPvPBuff: ${buffName} from ${generalName} adds ${additional} to ${score}`
              ...
              if (!UnitSchema.enum.percentage.localeCompare(tb.value.unit)) {
                const multiplier = SiegePvPAttackAttributeMultipliers?.Offensive.AllTroopAttack ?? 0;
                const additional = tb.value.number * multiplier;
                if (DEBUG_GBUFF) {
                  console.log(
                    `GroundPvPBuff: ${buffName} from ${generalName} adds ${additional} to ${score}`
            ...
            if (tb.condition !== undefined && tb.condition !== null && (
              ...
              if (!UnitSchema.enum.percentage.localeCompare(tb.value.unit)) {
                const multiplier = SiegePvPAttackAttributeMultipliers?.AttackingToughnessDebuff.ReduceEnemyAllDefense ?? 0;
                const additional = tb.value.number * multiplier;
                if (DEBUG_GBUFF) {
                  console.log(
                    `GroundPvPBuff: ${buffName} from ${generalName} adds ${additional} to ${score}`
              ...
              //I think all other conditions that matter have been checked
              if (!UnitSchema.enum.percentage.localeCompare(tb.value.unit)) {
                const multiplier = SiegePvPAttackAttributeMultipliers?.Toughness.AllTroopDefense ?? 0;
                const additional = tb.value.number * multiplier;
                if (DEBUG_GBUFF) {
                  console.log(
                    `GroundPvPBuff: ${buffName} from ${generalName} adds ${additional} to ${score}`
            ...
            if (tb.condition !== undefined && tb.condition !== null && (
              ...
              if (!UnitSchema.enum.percentage.localeCompare(tb.value.unit)) {
                const multiplier = SiegePvPAttackAttributeMultipliers?.AttackingToughnessDebuff.ReduceEnemyAllHP ?? 0;
                const additional = tb.value.number * multiplier;
                if (DEBUG_GBUFF) {
                  console.log(
                    `GroundPvPBuff: ${buffName} from ${generalName} adds ${additional} to ${score}`
              ...
              //I think all other conditions that matter have been checked
              if (!UnitSchema.enum.percentage.localeCompare(tb.value.unit)) {
                const multiplier = SiegePvPAttackAttributeMultipliers?.Toughness.AllTroopHP ?? 0;
                const additional = tb.value.number * multiplier;
                if (DEBUG_GBUFF) {
                  console.log(
                    `GroundPvPBuff: ${buffName} from ${generalName} adds ${additional} to ${score}`
          ...
          if (!Attribute.enum.Attack.localeCompare(tb.attribute)) {
            if (tb.condition !== undefined && tb.condition !== null && (
              ...
              if (!UnitSchema.enum.percentage.localeCompare(tb.value.unit)) {
                ...
                if (!ClassEnum.enum.Ground.localeCompare(tb.class)) {
                  multiplier = SiegePvPAttackAttributeMultipliers?.AttackingAttackDebuff.ReduceEnemyGroundAttack ?? 0;
                } else if (!ClassEnum.enum.Archers.localeCompare(tb.class)) {
                  multiplier = SiegePvPAttackAttributeMultipliers?.AttackingAttackDebuff.ReduceEnemyRangedAttack ?? 0;
                } else if (!ClassEnum.enum.Mounted.localeCompare(tb.class)) {
                  multiplier = SiegePvPAttackAttributeMultipliers?.AttackingAttackDebuff.ReduceEnemyMountedAttack ?? 0;
                } else if (!ClassEnum.enum.Siege.localeCompare(tb.class)) {
                  multiplier = SiegePvPAttackAttributeMultipliers?.AttackingAttackDebuff.ReduceEnemySiegeAttack ?? 0;
                }
                const additional = tb.value.number * multiplier;
                if (DEBUG_GBUFF) {
                  console.log(
              ...
              if (!UnitSchema.enum.percentage.localeCompare(tb.value.unit)) {
                let multiplier = 0;
                if (!ClassEnum.enum.Ground.localeCompare(tb.class)) {
                  multiplier = SiegePvPAttackAttributeMultipliers?.Offensive.GroundAttack ?? 0;
                } else if (!ClassEnum.enum.Archers.localeCompare(tb.class)) {
                  multiplier = SiegePvPAttackAttributeMultipliers?.Offensive.RangedAttack ?? 0;
                } else if (!ClassEnum.enum.Mounted.localeCompare(tb.class)) {
                  multiplier = SiegePvPAttackAttributeMultipliers?.Offensive.MountedAttack ?? 0;
                } else if (!ClassEnum.enum.Siege.localeCompare(tb.class)) {
                  multiplier = SiegePvPAttackAttributeMultipliers?.Offensive.SiegeAttack ?? 0;
                }
                const additional = tb.value.number * multiplier;
                if (DEBUG_GBUFF) {
                  console.log(
            ...
            if (tb.condition !== undefined && tb.condition !== null && (
              ...
              if (!UnitSchema.enum.percentage.localeCompare(tb.value.unit)) {
                let multiplier = 0;
                if (!ClassEnum.enum.Ground.localeCompare(tb.class)) {
                  multiplier = SiegePvPAttackAttributeMultipliers?.AttackingToughnessDebuff.ReduceEnemyGroundDefense ?? 0;
                } else if (!ClassEnum.enum.Archers.localeCompare(tb.class)) {
                  multiplier = SiegePvPAttackAttributeMultipliers?.AttackingToughnessDebuff.ReduceEnemyRangedDefense ?? 0;
                } else if (!ClassEnum.enum.Mounted.localeCompare(tb.class)) {
                  multiplier = SiegePvPAttackAttributeMultipliers?.AttackingToughnessDebuff.ReduceEnemyMountedDefense ?? 0;
                } else if (!ClassEnum.enum.Siege.localeCompare(tb.class)) {
                  multiplier = SiegePvPAttackAttributeMultipliers?.AttackingToughnessDebuff.ReduceEnemySiegeDefense ?? 0;
                }
                const additional = tb.value.number * multiplier;
                if (DEBUG_GBUFF) {
                  console.log(
              ...
              if (!UnitSchema.enum.percentage.localeCompare(tb.value.unit)) {
                ...
                let multiplier = 0;
                if (!ClassEnum.enum.Ground.localeCompare(tb.class)) {
                  multiplier = SiegePvPAttackAttributeMultipliers?.Toughness.GroundDefense ?? 0;
                } else if (!ClassEnum.enum.Archers.localeCompare(tb.class)) {
                  multiplier = SiegePvPAttackAttributeMultipliers?.Toughness.RangedDefense ?? 0;
                } else if (!ClassEnum.enum.Mounted.localeCompare(tb.class)) {
                  multiplier = SiegePvPAttackAttributeMultipliers?.Toughness.MountedDefense ?? 0;
                } else if (!ClassEnum.enum.Siege.localeCompare(tb.class)) {
                  multiplier = SiegePvPAttackAttributeMultipliers?.Toughness.SiegeDefense ?? 0;
                }
                const additional = tb.value.number * multiplier;
                if (DEBUG_GBUFF) {
                  console.log(
            ...
            if (tb.condition !== undefined && tb.condition !== null && (
              ...
              if (!UnitSchema.enum.percentage.localeCompare(tb.value.unit)) {
                let multiplier = 0;
                if (!ClassEnum.enum.Ground.localeCompare(tb.class)) {
                  multiplier = SiegePvPAttackAttributeMultipliers?.AttackingToughnessDebuff.ReduceEnemyGroundHP ?? 0;
                } else if (!ClassEnum.enum.Archers.localeCompare(tb.class)) {
                  multiplier = SiegePvPAttackAttributeMultipliers?.AttackingToughnessDebuff.ReduceEnemyRangedHP ?? 0;
                } else if (!ClassEnum.enum.Mounted.localeCompare(tb.class)) {
                  multiplier = SiegePvPAttackAttributeMultipliers?.AttackingToughnessDebuff.ReduceEnemyMountedHP ?? 0;
                } else if (!ClassEnum.enum.Siege.localeCompare(tb.class)) {
                  multiplier = SiegePvPAttackAttributeMultipliers?.AttackingToughnessDebuff.ReduceEnemySiegeHP ?? 0;
                }
                const additional = tb.value.number * multiplier;
                if (DEBUG_GBUFF) {
                  console.log(
              ...
              if (!UnitSchema.enum.percentage.localeCompare(tb.value.unit)) {
                let multiplier = 0;
                if (!ClassEnum.enum.Ground.localeCompare(tb.class)) {
                  multiplier = SiegePvPAttackAttributeMultipliers?.Toughness.GroundHP ?? 0;
                } else if (!ClassEnum.enum.Archers.localeCompare(tb.class)) {
                  multiplier = SiegePvPAttackAttributeMultipliers?.Toughness.RangedHP ?? 0;
                } else if (!ClassEnum.enum.Mounted.localeCompare(tb.class)) {
                  multiplier = SiegePvPAttackAttributeMultipliers?.Toughness.MountedHP ?? 0;
                } else if (!ClassEnum.enum.Siege.localeCompare(tb.class)) {
                  multiplier = SiegePvPAttackAttributeMultipliers?.Toughness.SiegeHP ?? 0;
                }
                const additional = tb.value.number * multiplier;
                if (DEBUG_GBUFF) {
                  console.log(
            ...
            if (!ClassEnum.enum.Archers.localeCompare(tb.class)) {
              if (!UnitSchema.enum.percentage.localeCompare(tb.value.unit)) {
                multiplier = SiegePvPAttackAttributeMultipliers?.Offensive.RangedRangeBonus ?? 0;

              }
            } else if (!ClassEnum.enum.Siege.localeCompare(tb.class)) {
              if (!UnitSchema.enum.percentage.localeCompare(tb.value.unit)) {
                multiplier = SiegePvPAttackAttributeMultipliers?.Offensive.SiegeRangeBonusPercent ?? 0;
              } else {
                multiplier = SiegePvPAttackAttributeMultipliers?.Offensive.SiegeRangeBonusFlat ?? 0;
              }
            }
            const additional = tb.value.number * multiplier
            if (DEBUG_GBUFF) {
