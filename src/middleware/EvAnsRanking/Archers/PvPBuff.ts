      ...
      if (tb.class === undefined || tb.class === null || !ClassEnum.enum.all.localeCompare(tb.class)) {
        ...
        if (tb.attribute === undefined || tb.attribute === null) {
          ...
          if (!Attribute.enum.March_Size_Capacity.localeCompare(tb.attribute)) {
            ...
            if (!UnitSchema.enum.percentage.localeCompare(tb.value.unit)) {
              const multiplier = RangedPvPAttackAttributeMultipliers?.Preservation.Death2Souls ?? 0;
              const additional = tb.value.number * multiplier;
              if (DEBUG_GBUFF) {
                console.log(
                  `GroundPvPBuff: ${buffName} from ${generalName} adds ${additional} to ${score}`
            ...
            if (!UnitSchema.enum.percentage.localeCompare(tb.value.unit)) {
              const multiplier = RangedPvPAttackAttributeMultipliers?.Preservation.Death2Wounded ?? 0;
              const additional = tb.value.number * multiplier;
              if (DEBUG_GBUFF) {
                console.log(
                  `GroundPvPBuff: ${buffName} from ${generalName} adds ${additional} to ${score}`
            ...
            if (tb.condition !== undefined && tb.condition !== null && (
              ...
              if (!UnitSchema.enum.percentage.localeCompare(tb.value.unit)) {
                ...
                const multiplier = RangedPvPAttackAttributeMultipliers?.AttackingAttackDebuff.ReduceAllAttack ?? 0;
                const additional = tb.value.number * multiplier;
                if (DEBUG_GBUFF) {
                  console.log(
                    `GroundPvPBuff: ${buffName} from ${generalName} adds ${additional} to ${score}`
            ...
            if (tb.condition !== undefined && tb.condition !== null && (
              ...
              //I think all other conditions that matter have been checked
              if (!UnitSchema.enum.percentage.localeCompare(tb.value.unit)) {
                const multiplier = RangedPvPAttackAttributeMultipliers?.Toughness.AllTroopDefense ?? 0;
                const additional = tb.value.number * multiplier;
                if (DEBUG_GBUFF) {
                  console.log(
                    `GroundPvPBuff: ${buffName} from ${generalName} adds ${additional} to ${score}`
            ...
            if (tb.condition !== undefined && tb.condition !== null && (
              ...
              //I think all other conditions that matter have been checked
              if (!UnitSchema.enum.percentage.localeCompare(tb.value.unit)) {
                const multiplier = RangedPvPAttackAttributeMultipliers?.Toughness.AllTroopHP ?? 0;
                const additional = tb.value.number * multiplier;
                if (DEBUG_GBUFF) {
                  console.log(
                    `GroundPvPBuff: ${buffName} from ${generalName} adds ${additional} to ${score}`
          ...
          if (!Attribute.enum.Attack.localeCompare(tb.attribute)) {
            if (tb.condition !== undefined && tb.condition !== null && (
              ...
              if (!UnitSchema.enum.percentage.localeCompare(tb.value.unit)) {
                ...
                if (!ClassEnum.enum.Ground.localeCompare(tb.class)) {
                  multiplier = RangedPvPAttackAttributeMultipliers?.AttackingAttackDebuff.ReduceEnemyGroundAttack ?? 0;
                } else if (!ClassEnum.enum.Archers.localeCompare(tb.class)) {
                  multiplier = RangedPvPAttackAttributeMultipliers?.AttackingAttackDebuff.ReduceEnemyRangedAttack ?? 0;
                } else if (!ClassEnum.enum.Mounted.localeCompare(tb.class)) {
                  multiplier = RangedPvPAttackAttributeMultipliers?.AttackingAttackDebuff.ReduceEnemyMountedAttack ?? 0;
                } else if (!ClassEnum.enum.Siege.localeCompare(tb.class)) {
                  multiplier = RangedPvPAttackAttributeMultipliers?.AttackingAttackDebuff.ReduceEnemySiegeAttack ?? 0;
                }
                const additional = tb.value.number * multiplier;
                if (DEBUG_GBUFF) {
                  console.log(
            ...
            if (tb.condition !== undefined && tb.condition !== null && (
              ...
              if (!UnitSchema.enum.percentage.localeCompare(tb.value.unit)) {
                ...
                let multiplier = 0;
                if (!ClassEnum.enum.Ground.localeCompare(tb.class)) {
                  multiplier = RangedPvPAttackAttributeMultipliers?.Toughness.GroundDefense ?? 0;
                } else if (!ClassEnum.enum.Archers.localeCompare(tb.class)) {
                  multiplier = RangedPvPAttackAttributeMultipliers?.Toughness.RangedDefense ?? 0;
                } else if (!ClassEnum.enum.Mounted.localeCompare(tb.class)) {
                  multiplier = RangedPvPAttackAttributeMultipliers?.Toughness.MountedDefense ?? 0;
                } else if (!ClassEnum.enum.Siege.localeCompare(tb.class)) {
                  multiplier = RangedPvPAttackAttributeMultipliers?.Toughness.SiegeDefense ?? 0;
                }
                const additional = tb.value.number * multiplier;
                if (DEBUG_GBUFF) {
                  console.log(
            ...
            if (tb.condition !== undefined && tb.condition !== null && (
              ...
              if (!UnitSchema.enum.percentage.localeCompare(tb.value.unit)) {
                let multiplier = 0;
                if (!ClassEnum.enum.Ground.localeCompare(tb.class)) {
                  multiplier = RangedPvPAttackAttributeMultipliers?.Toughness.GroundHP ?? 0;
                } else if (!ClassEnum.enum.Archers.localeCompare(tb.class)) {
                  multiplier = RangedPvPAttackAttributeMultipliers?.Toughness.RangedHP ?? 0;
                } else if (!ClassEnum.enum.Mounted.localeCompare(tb.class)) {
                  multiplier = RangedPvPAttackAttributeMultipliers?.Toughness.MountedHP ?? 0;
                } else if (!ClassEnum.enum.Siege.localeCompare(tb.class)) {
                  multiplier = RangedPvPAttackAttributeMultipliers?.Toughness.SiegeHP ?? 0;
                }
                const additional = tb.value.number * multiplier;
                if (DEBUG_GBUFF) {
                  console.log(
