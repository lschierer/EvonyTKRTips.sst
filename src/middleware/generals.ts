    ...
    const pairGenerals = () => {
        if (locals.ExtendedGenerals.length > 0) {
          ...
          [...generalBaseN].forEach((pPair) => {
            if (!pPair[0].general.name.localeCompare(pPair[1].general.name)) {
              return;
            }
            if (locals.ConflictData.length > 0) {
              const relConflicts = locals.ConflictData.some((cDatum) => {
                ...
                if (c.includes(pPair[0].general.name)) {
                  if (c.includes(pPair[1])) {
                    return true;
                  }
                }
                return false;
                ...
                if (
                  !locals.CachedPairs.some((thisPair) => {
                    if (thisPair !== undefined && thisPair !== null) {
                      if (
                        thisPair[0].general.name.localeCompare(
                          pPair[0].general.name
                        )
                      ) {
                        if (
                          thisPair[1].general.name.localeCompare(
