        ...
        if (tb.attribute === undefined || tb.attribute === null) {
          ...
          if (!Attribute.enum.March_Size_Capacity.localeCompare(tb.attribute)) {
            if (!UnitSchema.enum.percentage.localeCompare(tb.value.unit)) {
              const multiplier = MountedPvPAttackAttributeMultipliers?.Offensive.MarchSizeIncrease ?? 0;
            ...
            if (tb.condition !== undefined && tb.condition !== null && (
              ...
              if (!UnitSchema.enum.percentage.localeCompare(tb.value.unit)) {
                const multiplier = MountedPvPAttackAttributeMultipliers?.Offensive.AllTroopAttack ?? 0;
                const additional = tb.value.number * multiplier;
                if (DEBUG_GBUFF) {
                  console.log(
                    `GroundPvPBuff: ${buffName} from ${generalName} adds ${additional} to ${score}`
            ...
            if (tb.condition !== undefined && tb.condition !== null && (
              ...
              if (!UnitSchema.enum.percentage.localeCompare(tb.value.unit)) {
                const multiplier = MountedPvPAttackAttributeMultipliers?.AttackingToughnessDebuff.ReduceEnemyAllDefense ?? 0;
                const additional = tb.value.number * multiplier;
                if (DEBUG_GBUFF) {
                  console.log(
                    `GroundPvPBuff: ${buffName} from ${generalName} adds ${additional} to ${score}`
            ...
            if (tb.condition !== undefined && tb.condition !== null && (
              ...
              if (!UnitSchema.enum.percentage.localeCompare(tb.value.unit)) {
                const multiplier = MountedPvPAttackAttributeMultipliers?.AttackingToughnessDebuff.ReduceEnemyAllHP ?? 0;
                const additional = tb.value.number * multiplier;
                if (DEBUG_GBUFF) {
                  console.log(
                    `GroundPvPBuff: ${buffName} from ${generalName} adds ${additional} to ${score}`
          ...
          if (!Attribute.enum.Attack.localeCompare(tb.attribute)) {
            if (tb.condition !== undefined && tb.condition !== null && (
              ...
              if (!UnitSchema.enum.percentage.localeCompare(tb.value.unit)) {
                let multiplier = 0;
                if (!ClassEnum.enum.Ground.localeCompare(tb.class)) {
                  multiplier = MountedPvPAttackAttributeMultipliers?.Offensive.GroundAttack ?? 0;
                } else if (!ClassEnum.enum.Archers.localeCompare(tb.class)) {
                  multiplier = MountedPvPAttackAttributeMultipliers?.Offensive.RangedAttack ?? 0;
                } else if (!ClassEnum.enum.Mounted.localeCompare(tb.class)) {
                  multiplier = MountedPvPAttackAttributeMultipliers?.Offensive.MountedAttack ?? 0;
                } else if (!ClassEnum.enum.Siege.localeCompare(tb.class)) {
                  multiplier = MountedPvPAttackAttributeMultipliers?.Offensive.SiegeAttack ?? 0;
                }
                const additional = tb.value.number * multiplier;
                if (DEBUG_GBUFF) {
                  console.log(
            ...
            if (tb.condition !== undefined && tb.condition !== null && (
              ...
              if (!UnitSchema.enum.percentage.localeCompare(tb.value.unit)) {
                let multiplier = 0;
                if (!ClassEnum.enum.Ground.localeCompare(tb.class)) {
                  multiplier = MountedPvPAttackAttributeMultipliers?.AttackingToughnessDebuff.ReduceEnemyGroundDefense ?? 0;
                } else if (!ClassEnum.enum.Archers.localeCompare(tb.class)) {
                  multiplier = MountedPvPAttackAttributeMultipliers?.AttackingToughnessDebuff.ReduceEnemyRangedDefense ?? 0;
                } else if (!ClassEnum.enum.Mounted.localeCompare(tb.class)) {
                  multiplier = MountedPvPAttackAttributeMultipliers?.AttackingToughnessDebuff.ReduceEnemyMountedDefense ?? 0;
                } else if (!ClassEnum.enum.Siege.localeCompare(tb.class)) {
                  multiplier = MountedPvPAttackAttributeMultipliers?.AttackingToughnessDebuff.ReduceEnemySiegeDefense ?? 0;
                }
                const additional = tb.value.number * multiplier;
                if (DEBUG_GBUFF) {
                  console.log(
            ...
            if (tb.condition !== undefined && tb.condition !== null && (
              ...
              if (!UnitSchema.enum.percentage.localeCompare(tb.value.unit)) {
                let multiplier = 0;
                if (!ClassEnum.enum.Ground.localeCompare(tb.class)) {
                  multiplier = MountedPvPAttackAttributeMultipliers?.AttackingToughnessDebuff.ReduceEnemyGroundHP ?? 0;
                } else if (!ClassEnum.enum.Archers.localeCompare(tb.class)) {
                  multiplier = MountedPvPAttackAttributeMultipliers?.AttackingToughnessDebuff.ReduceEnemyRangedHP ?? 0;
                } else if (!ClassEnum.enum.Mounted.localeCompare(tb.class)) {
                  multiplier = MountedPvPAttackAttributeMultipliers?.AttackingToughnessDebuff.ReduceEnemyMountedHP ?? 0;
                } else if (!ClassEnum.enum.Siege.localeCompare(tb.class)) {
                  multiplier = MountedPvPAttackAttributeMultipliers?.AttackingToughnessDebuff.ReduceEnemySiegeHP ?? 0;
                }
                const additional = tb.value.number * multiplier;
                if (DEBUG_GBUFF) {
                  console.log(
            ...
            if (!ClassEnum.enum.Archers.localeCompare(tb.class)) {
              if (!UnitSchema.enum.percentage.localeCompare(tb.value.unit)) {
                multiplier = MountedPvPAttackAttributeMultipliers?.Offensive.RangedRangeBonus ?? 0;

              }
            } else if (!ClassEnum.enum.Siege.localeCompare(tb.class)) {
              if (!UnitSchema.enum.percentage.localeCompare(tb.value.unit)) {
                multiplier = MountedPvPAttackAttributeMultipliers?.Offensive.SiegeRangeBonusPercent ?? 0;
              } else {
                multiplier = MountedPvPAttackAttributeMultipliers?.Offensive.SiegeRangeBonusFlat ?? 0;
              }
            }
            const additional = tb.value.number * multiplier
            if (DEBUG_GBUFF) {
