    ...
    (
      ...
      if (tb !== undefined && tb.value !== undefined) {
        ...
        if (tb.class === undefined || tb.class === null) {
          if (DEBUG_GBUFF) {
            console.log(
              `buff ${buffName} for ${generalName} check for no class`
            );
          }
          ...
          if (tb.attribute !== undefined) {
            if (DEBUG_GBUFF) {
              console.log(
                `buff ${buffName} for ${generalName} check for no attribute`
              );
            }
            if (
              !tb.attribute.localeCompare(Attribute.enum.March_Size_Capacity)
            ) {
              if (tb.value !== undefined && tb.value !== null) {
                if (!tb.value.unit.localeCompare(UnitSchema.enum.percentage)) {
                  const multiplier =
                    AttributeMultipliers[
                      "Rally Owner PvP"
                    ]?.Offensive.MarchSizeIncrease.get(
                  ...
                  if (tb.value !== undefined && tb.value !== null) {
                    if (
                      ...
                      const multiplier =
                        AttributeMultipliers[
                          "Rally Owner PvP"
                        ]?.AttackingAttackDebuff.ReduceAllAttack.get(
                          generalSpecialists.enum.Ground
                        ) ?? 1;
                      const additional = tb.value.number * multiplier;
                      if (DEBUG_GBUFF) {
              ...
              //people in your rally will essentially act as more layers.
              //count this slightly. 
              if (tb.value !== undefined && tb.value !== null) {
                if (!tb.value.unit.localeCompare(UnitSchema.enum.percentage)) {
                  const multiplier = 
              ...
              //Wounded might be better than dead, but you still have to heal them.  I'm discounting this more. 
              if (tb.value !== undefined && tb.value !== null) {
                if (!tb.value.unit.localeCompare(UnitSchema.enum.percentage)) {
                  if (tb.condition !== undefined && tb.condition !== null) {
                    if (
                      ...
                      const multiplier =
                        AttributeMultipliers[
                          "Rally Owner PvP"
                        ]?.Preservation.Death2WoundedWhenAttacking.get(
                          generalSpecialists.enum.Ground
                        ) ?? 0;
                      const additional = tb.value.number * multiplier;
                      if (DEBUG_GBUFF) {
                    ...
                    const multiplier =
                      AttributeMultipliers[
                        "Rally Owner PvP"
                      ]?.Preservation.Death2Wounded.get(
                        generalSpecialists.enum.Ground
                      ) ?? 0;
                    const additional = tb.value.number * multiplier;
                    if (DEBUG_GBUFF) {
              ...
              if (tb.value !== undefined && tb.value !== null) {
                if (!tb.value.unit.localeCompare(UnitSchema.enum.percentage)) {
                  const multiplier =
                    AttributeMultipliers[
                      "Rally Owner PvP"
                    ]?.Toughness.GroundHP.get(generalSpecialists.enum.Ground) ??
                    1;
                  const additional = tb.value.number * multiplier;
                  if (DEBUG_GBUFF) {
                    console.log(
              ...
              if (tb.value !== undefined && tb.value !== null) {
                if (!tb.value.unit.localeCompare(UnitSchema.enum.percentage)) {
                  const multiplier =
                    AttributeMultipliers[
                      "Rally Owner PvP"
                    ]?.Toughness.RangedHP.get(generalSpecialists.enum.Ground) ??
                    1;
                  const additional = tb.value.number * multiplier;
                  if (DEBUG_GBUFF) {
                    console.log(
              ...
              if (tb.value !== undefined && tb.value !== null) {
                if (!tb.value.unit.localeCompare(UnitSchema.enum.percentage)) {
                  const multiplier =
                    AttributeMultipliers[
                      "Rally Owner PvP"
                    ]?.Toughness.SiegeHP.get(generalSpecialists.enum.Ground) ??
                    1;
                  const additional = tb.value.number * multiplier;
                  if (DEBUG_GBUFF) {
                    console.log(
