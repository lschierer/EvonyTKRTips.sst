...
const Basic = z
  ...
  .implement((eg: ExtendedGeneralType) => {
    ...
    const attackMultiplier = RangedPvPAttackAttributeMultipliers?.Offensive.AllTroopAttack ?? 1;
    const defenseMultiplier = RangedPvPAttackAttributeMultipliers?.Toughness.AllTroopDefense ?? 1;
    const HPMultipler = RangedPvPAttackAttributeMultipliers?.Toughness.AllTroopHP ?? 1;
    const PoliticsMultipler = RangedPvPAttackAttributeMultipliers?.Preservation.Death2Wounded ?? 1;
