...
export class CompareMayors extends LitElement {
  ...
  static styles = css`
    ...
    .table-container #Name {
      width: clamp(
        ...
        var(--spectrum-global-dimension-size-1200),
        12%
      );
      flex: 1.5 1 auto;
    }
    ...
    if (this.renderRoot) {
      ...
      if (this.table !== undefined && this.table !== null) {
        this.table.renderItem = (item, index) => {
          return html`${this.ids.map((id) => {
            return html`<sp-table-cell
              id=${id.replace(' ', '_')}
              dir="ltr"
              role="gridcell"
              >${item[id]}</sp-table-cell
            >`;
          })} `;
        };
  ...
  async willUpdate(changedProperties: PropertyValues<this>) {
    if (changedProperties.has('DataUrl')) {
      ...
      if (this.renderRoot) {
        const table: Table | null = this.renderRoot.querySelector(
          '#' + this.tableName
        );
        if (table) {
          ...
          <sp-table-head>
            ${this.ids.map((id) => {
              return html`<sp-table-head-cell
                value=${id.replace(/ /g, '_')}
                sortable
                sort-direction="desc"
                sort-key=${id}
                >${id}</sp-table-head-cell
              >`;
            })}
          </sp-table-head>
        </sp-table>
