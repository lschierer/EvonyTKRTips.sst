...
const buffReductionLogic = (
  ...
  total = tbfo.Attack(buffName, eg.name, ab, bp, useCase, am);
  if (DEBUG) {
    console.log(`CovBuff Attack adding ${total} to: ${ba}`);
  }
  ba += total;
  total = tbfo.MarchSize(buffName, eg.name, ab, bp, useCase, am);
  if (DEBUG) {
    console.log(`CovBuff MarchSize adding ${total} to: ${ba}`);
  }
  ba += total;

  total = tbfo.DeHP(buffName, eg.name, ab, bp, useCase, am);
  if (DEBUG) {
    console.log(`CovBuff DeHP adding ${total} to: ${ba}`);
  }
  ba += total;
  total = tbfo.DeDefense(buffName, eg.name, ab, bp, useCase, am);
  if (DEBUG) {
  ...
  total = tbfo.Debilitation(buffName, eg.name, ab, bp, useCase, am);
  if (DEBUG) {
    console.log(`CovBuff Debilitation adding ${total} to: ${ba}`);
  }
  ba += total;
  total = tbfo.Preservation(buffName, eg.name, ab, bp, useCase, am);
  if (DEBUG) {
    console.log(`CovBuff Preservation adding ${total} to: ${ba}`);
  }
  ba += total;
