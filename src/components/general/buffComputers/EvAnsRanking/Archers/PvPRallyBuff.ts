...
const PvPRallyBuffClassCheck = z.function()
      ...
      if (tb.value !== null && tb.value !== undefined) {
        if (!UnitSchema.enum.percentage.localeCompare(tb.value.unit)) {
          if (tb.class !== null && tb.class !== undefined) {
            ...
            multiplier = RangedPvPAttackAttributeMultipliers?.Offensive.RallyCapacity ?? 0;
          }
...
export const PvPRallyBuff = z.function()
      ...
      if (tb === null || tb === undefined || iv === null || iv === undefined) {
        ...
        if (tb?.value === undefined || tb.value === null) {
          ...
          if (tb.attribute === undefined || tb.attribute === null) {
            if (DEBUGRC) {
              console.log(`PvPRallyBuff: ${generalName}: ${buffName} has null attribute`);
            }
            return score;
          } else if (Attribute.enum.Rally_Capacity.localeCompare(tb.attribute)) {
            if (DEBUGRC) {
              console.log(`PvPRallyBuff: ${generalName}: ${buffName} is not an attack buff`);
            }
            return score;
          } else {
            ...
            if (tb.condition !== null && tb.condition !== undefined) {
              if (checkInvalidConditions(tb, iv)) {
                ...
                (tb.condition.includes(Condition.enum.Enemy) ||
                  ...
                  if (DEBUGRC) {
                    console.log(`PvPRallyBuff: ${generalName}: ${buffName} detected attack debuff`);
                  }
                  return 0;
                } else {
