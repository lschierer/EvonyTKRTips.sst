...
const PvPRangeBuffClassCheck = z.function()
...
export const PvPRangeBuff = z.function()
      ...
      if (tb === null || tb === undefined || iv === null || iv === undefined) {
        ...
        if (tb?.value === undefined || tb.value === null) {
          ...
          if (tb.attribute === undefined || tb.attribute === null) {
            if (DEBUGRB) {
              console.log(`PvPRangeBuff: ${generalName}: ${buffName} has null attribute`);
            }
            return score;
          } else if (Attribute.enum.Range.localeCompare(tb.attribute)) {
            if (DEBUGRB) {
              console.log(`PvPRangeBuff: ${generalName}: ${buffName} is not an attack buff`);
            }
            return score;
          } else {
            ...
            if (tb.condition !== null && tb.condition !== undefined) {
              if (checkInvalidConditions(tb, iv)) {
                ...
                (tb.condition.includes(Condition.enum.Enemy) ||
                  ...
                  if (DEBUGRB) {
                    console.log(`PvPRangeBuff: ${generalName}: ${buffName} detected attack debuff`);
                  }
                  return 0;
                } else {
