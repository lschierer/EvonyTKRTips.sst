        ...
        if (!UnitSchema.enum.percentage.localeCompare(tb.value.unit)) {
          if (tb.class !== null && tb.class !== undefined) {
            if (!ClassEnum.enum.Archers.localeCompare(tb.class)) {
              multiplier =
                GroundPvPAttackAttributeMultipliers?.Toughness.RangedHP ?? 0;
            } else if (!ClassEnum.enum.Ground.localeCompare(tb.class)) {
              multiplier =
                GroundPvPAttackAttributeMultipliers?.Toughness.GroundHP ?? 0;
            } else if (!ClassEnum.enum.Mounted.localeCompare(tb.class)) {
              multiplier =
                GroundPvPAttackAttributeMultipliers?.Toughness.MountedHP ?? 0;
            } else if (!ClassEnum.enum.Siege.localeCompare(tb.class)) {
              multiplier =
                GroundPvPAttackAttributeMultipliers?.Toughness.SiegeHP ?? 0;
            } else {
              multiplier = 0;
            }
          } else {
            multiplier =
              GroundPvPAttackAttributeMultipliers?.Toughness.AllTroopHP ?? 0;
          }
