        ...
        if (
          ...
          if (tb.attribute === undefined || tb.attribute === null) {
            ...
            if (
              ...
              if (!UnitSchema.enum.percentage.localeCompare(tb.value.unit)) {
                const multiplier =
                  SiegePvPAttackAttributeMultipliers?.Preservation
                    .Death2Souls ?? 0;
                const additional = tb.value.number * multiplier;
                if (DEBUG_GBUFF) {
                  console.log(
              ...
              if (
                ...
                if (!UnitSchema.enum.percentage.localeCompare(tb.value.unit)) {
                  const multiplier =
                    SiegePvPAttackAttributeMultipliers?.Toughness
                      .AllTroopDefense ?? 0;
                  const additional = tb.value.number * multiplier;
                  if (DEBUG_GBUFF) {
                    console.log(
              ...
              if (
                ...
                if (!UnitSchema.enum.percentage.localeCompare(tb.value.unit)) {
                  const multiplier =
                    SiegePvPAttackAttributeMultipliers?.Toughness.AllTroopHP ??
                    0;
                  const additional = tb.value.number * multiplier;
                  if (DEBUG_GBUFF) {
                    console.log(
            ...
            if (!Attribute.enum.Attack.localeCompare(tb.attribute)) {
              ...
              if (
                ...
                if (!UnitSchema.enum.percentage.localeCompare(tb.value.unit)) {
                  ...
                  if (!ClassEnum.enum.Ground.localeCompare(tb.class)) {
                    multiplier =
                      SiegePvPAttackAttributeMultipliers?.Toughness.GroundHP ??
                      0;
                  } else if (!ClassEnum.enum.Archers.localeCompare(tb.class)) {
                    multiplier =
                      SiegePvPAttackAttributeMultipliers?.Toughness.RangedHP ??
                      0;
                  } else if (!ClassEnum.enum.Mounted.localeCompare(tb.class)) {
                    multiplier =
                      SiegePvPAttackAttributeMultipliers?.Toughness.MountedHP ??
                      0;
                  } else if (!ClassEnum.enum.Siege.localeCompare(tb.class)) {
                    multiplier =
                      SiegePvPAttackAttributeMultipliers?.Toughness.SiegeHP ??
                      0;
                  }
                  const additional = tb.value.number * multiplier;
                  if (DEBUG_GBUFF) {
