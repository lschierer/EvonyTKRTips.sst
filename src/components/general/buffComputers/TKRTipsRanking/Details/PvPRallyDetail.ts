...
import { RallyBuff } from './RallyBuff'
...
export const PvPRallyDetail = z.function()
  ...
  .implement((eg: ExtendedGeneralType, bp: BuffParamsType, am: AttributeMultipliersType, useCase: generalUseCaseType, display: DisplayType) => {
    ...
    const bss =  SkillBookBuffs(eg, useCase, bp, am, RallyBuff);
    if(DEBUG) {
      console.log(`bss for ${eg.name}: ${bss}`)
    }
    const speciality = SpecialityBuffs(eg, useCase, bp, am, RallyBuff);
    const covenant = CovenantBuffs(eg, useCase, bp, am, RallyBuff);
    let  aes = 0;
    if(display.localeCompare(Display.enum.secondary)) {
      aes = AscendingBuffs(eg, useCase, bp, am, RallyBuff);
    }
