...
const EvAnsBasic = z
  ...
  .implement((eg: ExtendedGeneralType) => {
    ...
    const attackMultiplier =
      SiegePvPAttackAttributeMultipliers?.Offensive.AllTroopAttack ?? 1;
    const defenseMultiplier =
      SiegePvPAttackAttributeMultipliers?.Toughness.AllTroopDefense ?? 1;
    const HPMultiplier =
      SiegePvPAttackAttributeMultipliers?.Toughness.AllTroopHP ?? 1;
    const PoliticsMultiplier =
      SiegePvPAttackAttributeMultipliers?.Preservation.Death2Wounded ?? 1;
...
export const SiegePvPAttack = z
  ...
  .implement(
    (eg: ExtendedGeneralType, display: DisplayType, bp: BuffParamsType) => {
      if (DEBUG) {
        console.log(`${eg.name}: EvAnsSiegePvPAttack starting`);
      }
      ...
      if (DEBUG) {
        console.log(`EvAnsSiegePvPAttack: ${eg.name}: BSS: ${BSS}`);
        console.log(
          `EvAnsSiegePvPAttack: ${eg.name}: specialities: ${specialities}`
        );
        console.log(`EvAnsSiegePvPAttack: ${eg.name}: BAS: ${BAS}`);
        console.log(`EvAnsSiegePvPAttack: ${eg.name}: AES: ${AES}`);
        console.log(
          `EvAnsSiegePvPAttack: ${eg.name}: TLGS (BSS & 34SS): ${TLGS}`
        );
      }
