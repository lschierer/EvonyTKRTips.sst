        ...
        if (!UnitSchema.enum.percentage.localeCompare(tb.value.unit)) {
          if (tb.class !== null && tb.class !== undefined) {
            if (!ClassEnum.enum.Archers.localeCompare(tb.class)) {
              multiplier =
                MountedPvPAttackAttributeMultipliers?.Toughness.RangedHP ?? 0;
            } else if (!ClassEnum.enum.Ground.localeCompare(tb.class)) {
              multiplier =
                MountedPvPAttackAttributeMultipliers?.Toughness.GroundHP ?? 0;
            } else if (!ClassEnum.enum.Mounted.localeCompare(tb.class)) {
              multiplier =
                MountedPvPAttackAttributeMultipliers?.Toughness.MountedHP ?? 0;
            } else if (!ClassEnum.enum.Siege.localeCompare(tb.class)) {
              multiplier =
                MountedPvPAttackAttributeMultipliers?.Toughness.SiegeHP ?? 0;
            } else {
              multiplier = 0;
            }
          } else {
            multiplier =
              MountedPvPAttackAttributeMultipliers?.Toughness.AllTroopHP ?? 0;
          }
