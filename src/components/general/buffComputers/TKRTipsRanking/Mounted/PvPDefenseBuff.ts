        ...
        if (!UnitSchema.enum.percentage.localeCompare(tb.value.unit)) {
          if (tb.class !== null && tb.class !== undefined) {
            if (!ClassEnum.enum.Archers.localeCompare(tb.class)) {
              multiplier =
                MountedPvPAttackAttributeMultipliers?.Toughness.RangedDefense ??
                0;
            } else if (!ClassEnum.enum.Ground.localeCompare(tb.class)) {
              multiplier =
                MountedPvPAttackAttributeMultipliers?.Toughness.GroundDefense ??
                0;
            } else if (!ClassEnum.enum.Mounted.localeCompare(tb.class)) {
              multiplier =
                MountedPvPAttackAttributeMultipliers?.Toughness.MountedDefense ??
                0;
            } else if (!ClassEnum.enum.Siege.localeCompare(tb.class)) {
              multiplier =
                MountedPvPAttackAttributeMultipliers?.Toughness.SiegeDefense ??
                0;
            } else {
              multiplier = 0;
            }
            ...
            multiplier =
              MountedPvPAttackAttributeMultipliers?.Toughness.AllTroopDefense ??
              0;
          }
