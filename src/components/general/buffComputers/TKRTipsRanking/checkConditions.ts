...
export const checkInvalidConditions = z
  ...
  .returns(z.boolean())
  .implement((tb: BuffType, iv: BuffParamsType,useCase: generalUseCaseType, debuff: boolean) => {
    if (tb.condition !== undefined && tb.condition !== null) {
      ...
      if(!useCase.localeCompare(generalUseCase.enum.Attack)) {
        ...
        InvalidConditions.push(Condition.enum.When_City_Mayor_for_this_SubCity);
        InvalidConditions.push(Condition.enum.When_Defending_Outside_The_Main_City);
        ...
        InvalidConditions.push(Condition.enum.When_The_Main_Defense_General);
        InvalidConditions.push(Condition.enum.When_an_officer);
        InvalidConditions.push(Condition.enum.brings_dragon_or_beast_to_attack);
