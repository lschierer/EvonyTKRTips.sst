...
export const TKRTipsArchersPvPToughness = z
  .function()
  ...
  .implement(
    (eg: ExtendedGeneralType, display: DisplayType, bp: BuffParamsType) => {
      if (DEBUG) {
        console.log(`${eg.name}: EvAnsArchersPvPToughness starting`);
      }
      ...
      if (DEBUG) {
        console.log(`ArchersPvPToughness: ${eg.name}: BSS: ${BSS}`);
        console.log(
          `ArchersPvPToughness: ${eg.name}: specialities: ${specialities}`
        );
        console.log(`ArchersPvPToughness: ${eg.name}: BAS: ${BAS}`);
        console.log(`ArchersPvPToughness: ${eg.name}: AES: ${AES}`);
        console.log(
          `ArchersPvPToughness: ${eg.name}: TLGS (BSS & 34SS): ${TLGS}`
        );
      }
