...
export class GridGeneral extends SizedMixin(SpectrumElement, {
  noDefaultSize: true,
}) {
  ...
  @state()
  private _eg: ExtendedGeneralType | null = null;
  ...
  .implement(( display, BP: BuffParamsType) => {
    if(this.general === null || this._eg === null) {
      ...
      const hashKey = GridGeneral.InvestmentOptions2Key(BP);
  ...
  protected async willUpdate(_changedProperties: PropertyValueMap<any> | Map<PropertyKey, unknown>): Promise<void> {
    ...
    if(_changedProperties.has('_eg')){
      if(DEBUG) {
        console.log(`change for _eg detected`)
      }
      if(
