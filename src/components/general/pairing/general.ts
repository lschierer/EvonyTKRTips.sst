...
export class DisplayGeneral {
    ...
    public async computeEvAnsRanking() {
        ...
        if(this.general !== undefined) {
            ...
            const scoreData = fetch(EvAnsUrl).then(async (r) => {
                if(r.ok) {
                ...
                console.log(`something went wrong fetching EvAnsUrl ${EvAnsUrl.toString()} error is ${e.message}`)
                return 0;
            })
            if(DEBUG ) console.log(`computeEvAnsRanking returning ${scoreData}`)
            this._EvAnsRanking = Math.floor(await scoreData);
            return scoreData;
        } else {
